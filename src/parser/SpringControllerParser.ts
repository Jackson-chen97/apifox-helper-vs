...
export class SpringControllerParser {
  ...
  private projectRootPath?: string; // 新增：项目根路径

  constructor(sourcePaths?: string[], projectName?: string, projectRootPath?: string) {
  ...
  async parse(): Promise<ApiEndpoint[]> {
    ...
    const controllerListener = new SpringControllerListener(openapiGenerator, this.projectName, this.projectRootPath);
  ...
  private async findControllerFiles(): Promise<vscode.Uri[]> {
    if (this.sourcePaths && this.sourcePaths.length > 0) {
      ...
      for (const sourcePath of this.sourcePaths) {
        ...
        // 找到 sourcePath 对应的工作区文件夹
        let targetFolder = workspaceFolders?.find(folder => {
          const folderPath = folder.uri.fsPath.replace(/\\/g, '/').replace(/\/$/, '');
          return normalizedSourcePath.startsWith(folderPath);
        });
        ...
        if (targetFolder) {
          const folderPath = targetFolder.uri.fsPath.replace(/\\/g, '/').replace(/\/$/, '');
          // 计算相对于工作区的路径
          let relativePath = normalizedSourcePath;
          if (normalizedSourcePath.startsWith(folderPath)) {
            relativePath = normalizedSourcePath.substring(folderPath.length).replace(/^\//, '');
          }
          ...
          const filteredFiles = found.filter(file => {
            ...
            return filePath.startsWith(normalizedSourcePath);
          });
