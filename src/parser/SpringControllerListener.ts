...
export class SpringControllerListener implements ParseTreeListener {
  ...
  swaggerTags: string | null; // 新增：swagger标签
  className: string | null; // 新增：类名
  ...
  constructor(openapiGenerator: any, projectName?: string, projectRootPath?: string) {
    ...
    this.projectRootPath = projectRootPath || ''; // 新增
    this.swaggerTags = null; // 新增
    this.className = null; // 新增
  }
  ...
  public setProjectRootPath(projectRootPath: string){
    this.projectRootPath = projectRootPath;
  }

  // 新增：提取swagger标签（兼容多版本）
  private extractSwaggerTags(annotations: any[]): string | null {
    // 1. 尝试 @Api(tags = "...") - Swagger 2 (Springfox)
    const apiAnnotation = annotations.find((annot: any) =>
        annot.text.startsWith("@Api(")
    );
    if (apiAnnotation) {
        const tagsMatch = apiAnnotation.text.match(/tags\s*=\s*"([^"]+)"/);
        if (tagsMatch) {
            return tagsMatch[1].trim();
        }
        // 备选：提取description作为标签
        const descMatch = apiAnnotation.text.match(/description\s*=\s*"([^"]+)"/);
        if (descMatch) {
            return descMatch[1].trim();
        }
    }
    
    // 2. 尝试 @Tag(name = "...") - Swagger 3 / OpenAPI 3 (SpringDoc)
    const tagAnnotation = annotations.find((annot: any) =>
        annot.text.startsWith("@Tag(")
    );
    if (tagAnnotation) {
        const nameMatch = tagAnnotation.text.match(/name\s*=\s*"([^"]+)"/);
        if (nameMatch) {
            return nameMatch[1].trim();
        }
    }
    
    return null; // 无标签，回退到类名
  }
  ...
  enterClassDeclaration(ctx: ClassDeclarationContext) {
    ...
    if (isController) {
      ...
      this.className = this.currentClass; // 存储类名

      // 提取swagger标签（优先级：@Api tags > @Tag name > 类名）
      const swaggerTags = this.extractSwaggerTags(classAnnotations);
      this.swaggerTags = swaggerTags; // 存储到成员变量

      // 如果有swagger标签，用作currentClassComment；否则用类名
      if (swaggerTags) {
        this.currentClassComment = swaggerTags;
      } else {
        // 没有swagger标签，尝试从@apiFolder注释中提取
        const apiFolder = this.getClassApiFolder(ctx);
        this.currentClassComment = apiFolder || this.currentClass;
      }
  ...
  enterMethodDeclaration(ctx: MethodDeclarationContext) {
    ...
    // 解析 @Operation 注解（Swagger 3 / SpringDoc）
    const operation = this.extractOperationAnnotation(methodAnnotations);
    ...
    // 优先使用 @Operation 注解（Swagger 3），其次使用 @ApiOperation（Swagger 2），最后使用注释
    if (operation) {
      // Swagger 3 @Operation 注解
      this.currentMethod.operation.summary = operation.summary || "";
      this.currentMethod.operation.description = operation.description || operation.summary || "";
    } else if (apiOperation) {
      // Swagger 2 @ApiOperation 注解
      // value 用作 summary
  ...
  /**
   * 解析 @Operation 注解（Swagger 3 / SpringDoc）
   * @example @Operation(summary = "查询AI商品匹配结果", description = "详细描述")
   */
  private extractOperationAnnotation(annotations: any[]): { summary?: string; description?: string } | null {
    const operationAnnotation = annotations.find((annot: any) =>
      annot.text.startsWith("@Operation(")
    );
    if (!operationAnnotation) return null;

    const text = operationAnnotation.text;
  ...
  exitMethodDeclaration(ctx: MethodDeclarationContext) {
    ...
    if (this.currentMethod) {
      try {
        ...
        this.endpoints.push({
          // ID中加入projectRootPath确保多项目时ID唯一
          id: `${this.projectRootPath || ''}::${this.currentMethod.method}  ${this.currentMethod.path}`,
          ...
          swaggerTags: this.swaggerTags || undefined, // 新增
          className: this.className || undefined, // 新增
          projectRootPath: this.projectRootPath || undefined, // 新增
